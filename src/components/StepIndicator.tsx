...
const StepIndicator = () => {
  ...
  return (
    <motion.div
      className="w-full py-6 px-4 md:px-8 bg-white rounded-xl shadow-md mb-8"
        ...
        {steps.map((step, index) => (
          <React.Fragment key={step.id}>
            <div className="flex flex-col items-center relative">
              ...
              >
                {index < currentStep - 1 ? <Check size={18} /> : step.id}
              </div>
              <p className={`text-xs md:text-sm mt-2 font-medium hidden md:block
