      ...
      <header className="backdrop-blur-md bg-[#FEF7CD]/60 shadow-sm fixed top-0 left-0 right-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2 group">
            <div className="bg-[#FEF7CD]/10 p-2 rounded-full group-hover:bg-[#FEF7CD]/20 transition-colors">
              <Ship className="h-6 w-6 text-[#FFC300]" />
            </div>
            <span className="text-xl font-bold text-[#FFC300] group-hover:text-[#FFD700] transition-colors">
              {t("app.name", "Retour")}
            </span>
          </Link>
          
          <nav className="flex items-center space-x-4">
            {isHomePage && <button onClick={() => setShowBookingModal(true)} className="inline-flex items-center gap-2 bg-[#FFC300] hover:bg-[#FFD700] text-white font-medium py-2 px-4 rounded-lg transition-all duration-300">
                {t("common.bookNow", "Book Now")}
              </button>}
            
            <Link to="/my-bookings" className={`flex items-center ${isMobile ? "" : "space-x-1"} text-[#FFC300] hover:text-[#FFD700] transition-colors font-medium`}>
